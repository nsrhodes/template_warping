...
project_dir = 'R:\DRS-KidsOPM\Natreg_n1\Notts_Einscans\';
wsl_project_dir = '/mnt/r/DRS-KidsOPM/Natreg_n1/Notts_Einscans/';
...
if template_mesh
    ...
    ft_write_mri([mri_path mri_file(1:end-4) '_padded.nii'],mri_orig,'dataformat','nifti')
    end 
    cfg = [];
...
%% Convert einscan units - if using coreg app this is not needed
% 
...
einscan2template = load([einscanpath mri_file(1:8) 'template_points_einscan2template_transform.txt']); % read in transformation text file from coreg app
...
if crop
    % Define line to crop
    for i = 1:im_dim(2)
        template_image_filled(:,i,1:ceil(-0.13*i+92))=0;
    end
end
% plot patch to inspect
figure
...
str1 = [wsl_fsl_dir,'/bin/flirt -in ',wsl_project_dir, sub(1) '/' sub '_template_img.nii -ref ',wsl_project_dir, sub(1) '/' sub  '_einscan_img.nii -out ',wsl_project_dir, sub(1) '/' sub  '_template_img_crg.nii -omat ',wsl_project_dir, sub(1) '/' sub '_template_img_crg.mat -bins 256 -cost corratio -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12 -interp trilinear'];
% second command - applying transformation from first command to the padded
% template mri
if padded
    str2 = [wsl_fsl_dir,'/bin/flirt -in ',wsl_project_dir,sub(1) '/' mri_file(1:end-4) '_padded.nii -applyxfm -init ',wsl_project_dir, sub(1) '/' sub  '_template_img_crg.mat -out ',wsl_project_dir, sub(1) '/' sub '_template_file_crg.nii -paddingsize 0.0 -interp trilinear -ref ',wsl_project_dir, sub(1) '/' mri_file(1:end-4) '_padded.nii'];
else
...
% Unzip warped mri
gunzip([einscanpath filename(1:7) '_template_file_crg.nii.gz'])
